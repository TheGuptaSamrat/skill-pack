Classification: training-derived-concepts
Source basis: FPSL TRAINING DOCUMENT.pdf
Trust usage: conceptual accounting process explanation
Do not use for: customer-specific accounting rules or local posting logic
Topics covered: accounting process model, flow explanation, example-driven framing

# Accounting Process Model

- Technical documentation should explain the accounting process model, not only the table or code layout.
- Loan and bond examples are useful conceptual anchors for describing process shape.
- These examples improve explanation quality but do not prove local configuration or custom mappings.
