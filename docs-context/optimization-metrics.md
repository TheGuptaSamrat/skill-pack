...
## CURRENT OPERATIONAL MEASUREMENT FRAMEWORK (2026-03-22)

The repository now has two deterministic measurement layers plus one human-evaluation layer:

### 1. Automated Context Metrics

Generated by:

- `scripts/measure_skill_efficiency.py`
- `scripts/audit_skill_pack_redundancy.py`
- `scripts/run-context-optimization.sh`

Current report outputs:

- `docs-context/indexes/skill-efficiency-report.md`
- `docs-context/indexes/skill-efficiency-report.json`
- `docs-context/indexes/redundancy-audit.md`
- `docs-context/indexes/redundancy-audit.json`

### 2. Human Output Evaluation

Captured in:

- `test-cases/feedback-template.md`

The feedback template now records:
...
- first-draft usability and prompt-turn count

### 3. Measurement Definitions

| Dimension | Measurement Source | Meaning |
|---|---|---|
| Token consumption | `skill-efficiency-report.md` | Estimated token cost using deterministic `chars / 4` heuristic |
| Context efficiency | `skill-efficiency-report.md` + developer feedback | Expansion ratio from entrypoint to full load, plus whether loaded context felt targeted |
| Output accuracy | `feedback-template.md` | Developer-rated accuracy plus factual correction count and anti-invention compliance |
| Usability | `feedback-template.md` | First-draft usability, ADT paste readiness, and usability rating |

### 4. Current Automated Snapshot

From `docs-context/indexes/skill-efficiency-report.md` and `docs-context/indexes/redundancy-audit.md`:

- Active skills measured: `11`
- Mean entrypoint tokens: `1043.8`
- Mean full-load tokens: `13186.1`
- Heaviest full-load skill: `cvpm` at `82658` estimated tokens
- Lightest full-load skill: `projections` at `1036` estimated tokens
- Reference markdown files: `55`
- Repeated section groups across skills: `0`
- Skill descriptions in recommended range: `11/11`

### 5. Integration Skill Baseline
...
- `integration` is heavier than `mapping`, `quality`, and `abap` at entrypoint, but materially lighter than `config`, `reconciliation`, and especially `cvpm` on full-load cost.
- The `6.34` expansion ratio is acceptable for a new focused skill, but should still be monitored if more references are added.
- The current baseline is strong enough to continue improving quality without creating a major context spike.

### 6. Source-of-Truth Note

The historical sections below are retained as phase history. For active efficiency review, use the generated reports in `docs-context/indexes/` instead of manually maintained line-count estimates.
